...
PING_SERVIDORES_DEFECTO = [
    ("Servidor AD",   "192.168.100.74"),
    ("Servidor WEB",  "192.168.100.79"),
...
def mostrar_menu():
    ...
    print(f"{VERDE}[3]{RESET} Ejecutar Ping Automático")      # <--- NUEVO Raphael 
    print(f"{VERDE}[4]{RESET} Ver Disponibilidad de Servicios") # <--- NUEVO Raphael
    print(f"{VERDE}[5]{RESET} Ver Logs de Alertas")
...
# ==============================================================================
# 7. MODULO DETECCIÓN DE DISPOSITIVOS DESCONOCIDOS
# ==============================================================================
# Lista blanca de dispositivos (MAC y/o IP permitidos)
...
def scan_network():
    ...
    for sent, received in result:
        ...
        if ip not in IP_MAC_CONFIANZA or IP_MAC_CONFIANZA[ip].lower() != mac:
            unknown_devices.append((ip, mac))
        # --- Guardar en logs automáticamente ---
    if unknown_devices:
        with open("logs.txt", "a", encoding="utf-8") as f:
            f.write("Dispositivos desconocidos detectados:\n")
            for ip, mac in unknown_devices:
                f.write(f"IP: {ip}, MAC: {mac}\n")
            f.write("-" * 40 + "\n")
# Para enviar correo de alerta por dispositivos desconocidos
    if unknown_devices:
    # Construir el cuerpo de forma segura (sin problemas de codificación)
        lineas = ["Se detectaron dispositivos desconocidos en la red:\n"]
        lineas += [f"- IP: {str(ip)}, MAC: {str(mac)}" for ip, mac in unknown_devices]
        cuerpo = "\n".join(lineas)

    enviar_email(
        asunto="Alerta de Seguridad - Dispositivo desconocido",
        cuerpo_texto=cuerpo,
        ruta_imagen=NOMBRE_LOGO
    )
...
def main():
    ...
    while True:
        ...
        if op == "1": 
            ...
            print(f"{ROJO}Por favor, seleccione un número del 1 al 6.{RESET}")
