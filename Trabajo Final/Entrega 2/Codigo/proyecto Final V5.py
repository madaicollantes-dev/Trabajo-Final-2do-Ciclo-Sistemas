...
IP_MAC_CONFIANZA = {
    ...
    "192.168.1.43": "a0:b1:c2:d3:e4:f5",  #Reemplazar por la ip de un equipo cualquiera que este conectado a la red la mac puede ser cualquiera (debe ser falsa) ya que esto activiara las alertas  
}
...
PING_SERVIDORES_DEFECTO = [
    ...
    ("Servidor WEB",  "192.168.1.43"), #Reemplazar por la ip de un equipo cualquiera que este conectado a la red la mac puede ser cualquiera (debe ser falsa) ya que esto activiara las alertas
...
def enviar_alerta_ping(nombre_servidor, ip_servidor):
    ...
    enviar_email_con_adjuntos(
    f"[ALERTA CRÍTICA] Servidor Caído - {nombre_servidor}",
    cuerpo,
    []
)
...
def generar_reporte_consolidado():
    ...
    # =============================
    # LOG PRINCIPAL (ARP, INTRUSOS, BW, PING)
    # =============================
    ...
    df = pd.DataFrame(eventos, columns=[
        "Fecha", "Hora", "Tipo_Alerta", "Detalle"
    ])

    excel_file = "reporte_consolidado.xlsx"
    ...
    enviar_email_con_adjuntos(
        ...
        "Reporte generado únicamente con eventos reales detectados por los módulos del sistema.\n\n"
        "Incluye dashboard con KPIs y gráficos de seguridad.",
        [excel_file]
    )
